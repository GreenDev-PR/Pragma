...
module.exports = function(sequelize, DataTypes) {
  var GoesData =  sequelize.define('GoesData', {
    ...
    classMethods: {
      ...
      getValuesBetween: function(variableName, startDate, endDate, coords) {
        return GoesData.findAll({
          where: {
            variableName: variableName,
            dataDate: {
              between: [startDate, endDate]
            },
            matrixColumn: locationMapper.getColumn(coords.longitude),
            matrixRow: locationMapper.getRow(coords.latitude)
          }
        });
      },
      ...
      getCumulativeEtData: function(sinceDate, latitude, longitude) {
        var sql = 'SELECT SUM(dataValue) FROM latestReferenceETData ' +
        'WHERE (dataDate BETWEEN  :since AND NOW()) AND ' +
        'matrixRow=:row AND matrixColumn=:column ';
